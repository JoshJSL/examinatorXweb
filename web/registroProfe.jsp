    ...
    if(!nombre.equals("")&&!pat.equals("")&&!mat.equals("")&&!boleta.equals("")&&!correo.equals("")&&!contr.equals("")&&!conContr.equals(""))
    {
        if(contr.equals(conContr)){
            ...
            if (vali.todo(nombre, pat, mat, boleta, correo, contr, conContr)) {
                    entidades.persona alumno=new entidades.persona("profe","0", boleta, contr, nombre, pat, mat, correo,"logoipn.jpg");
                    mal=alumno.guardarBD();
                    if(mal.equals("Usuario registrado")){
                        entidades.cMD5 md= new entidades.cMD5();
                        response.sendRedirect("confirma.jsp?mbdp5e="+md.md5(boleta)+"&"+"5cdomn="+md.md5(contr)+"&"+"qecorc="+md.md5("confirma")+"&"+"aop="+(9*156));
                    }
                }else{
                mal="Campos con caracteres invalidos :";
                mal += vali.letras(nombre) ? "" : " Nombre  ";
                mal += vali.letras(pat) ? "" : " Apelido Paterno ";
                mal += vali.letras(mat) ? "" : " Apelido Materno ";
                mal += vali.num(boleta) ? "" : " Clave ";
...
<body>
	...
	<div class="Contenido_e">
		...
		<div class="Crear">
                   ...
                   <h1 class="prt letra">Ingrese sus datos</h1><h1 class="pr letra">Profesor</h1>
				</div>
                    <div class="error">
                        <%=mal%> 
                    </div>
					...
					<tr>
                                            <td><li class="formi letra">Clave de profesor <span style="font-size: 10px;"> (10 digitos)</span>&nbsp;</li>	<input type="text" name="boleta" tabindex="4" class="texto"  onkeypress="validarBol();" required="true" id="Boleta" placeholder="Ingrese su calve" maxlength="10" value="<%=boleta%>"></td>
